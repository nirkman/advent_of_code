        ...
        for y in [self.y-1, self.y, self.y+1]:
            for x in [self.x-1, self.x, self.x+1]:
                if self.data[y][x].isdigit():
                    gears.append(int(self.look_back(x,y) + self.data[y][x] + self.look_forward(x,y)))
        print(gears)
        if len(gears) == 2:
            self.gear_ratios.append(gears[0] * gears[1])
...
print("")  
print("Gear ratios sum..: " + str(sum(schematic.gear_ratios)))  
